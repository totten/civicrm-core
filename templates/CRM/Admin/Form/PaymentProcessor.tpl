...
{literal}
  ...
  CRM.$(function($) {
    function initiateApiKey(processorId, initiatorUrl) {
      ...
      CRM.confirm({
        title: ts('External API Setup'),
        message: ts('This will redirect to another site to setup API keys.')
          + (unsavedChanges ? '<p>' + ts('<strong>Warning</strong>: Unsaved changes will be lost.') + '</p>' : '')
      }).on('crmConfirm:yes', function() {
        window.location = initiatorUrl;
      });
      return false;
    }
