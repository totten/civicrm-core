...
class CRM_Mailing_ActionTokens extends \Civi\Token\AbstractTokenSubscriber {
  ...
  public function __construct() {
    ...
    parent::__construct('action', array(
      ...
      'trackOpenUrl' => ts('Track Open URL (Action)'),
      'trackOpenHtml' => ts('Track Open HTML (Action)'),
    ));
  }
    ...
    switch ($field) {
      case 'trackOpenUrl':
        $row->format('text/plain')
          ->tokens($entity, $field, $this->getTrackOpenUrl($row));
        break;

      case 'trackOpenHtml':
        $row->format('text/plain')->tokens($entity, $field, '');
        $row->format('text/html')->tokens($entity, $field,
          '<img src="' . $this->getTrackOpenUrl($row) . "\" width='1' height='1' alt='' border='0'>");
