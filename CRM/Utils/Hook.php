...
abstract class CRM_Utils_Hook {
  ...
  public static function managed(&$entities, ?array $modules = NULL) {
    self::singleton()->invoke(['entities', 'modules'], $entities,
      $modules, self::$_nullObject, self::$_nullObject, self::$_nullObject, self::$_nullObject,
      'civicrm_managed'
    );
    if ($modules !== NULL) {
      $entities = array_filter($entities, function($decl) use ($modules) {
        return in_array($decl['module'], $modules);
      });
    }
  }
