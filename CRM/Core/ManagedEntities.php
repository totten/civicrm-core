...
class CRM_Core_ManagedEntities {
   ...
   *
   * @param string $moduleName
   *   Limit to a single module.
   * @throws \CRM_Core_Exception
   */
  public function reconcile($moduleName = NULL) {
    $this->loadDeclarations($moduleName);
    $this->loadManagedEntityActions($moduleName);
  ...
  private function getManagedAction(string $action): array {
    return CRM_Utils_Array::findAll($this->managedActions, ['managed_action' => $action]);
  }
   ...
   *
   * @param string $moduleName
   *   Limit reconciliation to a single module.
   */
  protected function loadDeclarations($moduleName = NULL): void {
    $declarations = [];
    // Exclude components if given a module name.
    if (!$moduleName || $moduleName === 'civicrm') {
      foreach (CRM_Core_Component::getEnabledComponents() as $component) {
        $declarations = array_merge($declarations, $component->getManagedEntities());
      }
    }
    CRM_Utils_Hook::managed($declarations, $moduleName ? [$moduleName] : NULL);
   ...
   * @param string $moduleName
   */
  protected function loadManagedEntityActions($moduleName = NULL): void {
