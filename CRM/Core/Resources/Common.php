...
class CRM_Core_Resources_Common {
  ...
  /**
   * This is a bundle of resources used on some non-Civi pages (wherein Civi
   * may be mixed-in).
   *
   * @param string $region
   * @return \CRM_Core_Resources_Bundle
   */
  public static function createStyleBundle($region) {
    ...
    if (!empty($config->customCSSURL)) {
      ...
      $bundle->addStyleUrl($customCSSURL, ['weight' => 99]);
    }
    if (!Civi::settings()->get('disable_core_css')) {
      $bundle->addStyleFile('civicrm', 'css/civicrm.css', ['weight' => -99]);
    }
    // crm-i.css added ahead of other styles so it can be overridden by FA.
    $bundle->addStyleFile('civicrm', 'css/crm-i.css', ['weight' => -101]);

    return self::useRegion($bundle, $region);
  }

  /**
   * This is a bundle of resources shared by Civi pages (ie pages where Civi
   * controls rendering).
   *
   * @param string $region
   * @return \CRM_Core_Resources_Bundle
   */
  public static function createFullBundle($region = 'html-header') {
    $bundle = new CRM_Core_Resources_Bundle();
    $config = CRM_Core_Config::singleton();

    // Add resources from coreResourceList
    $jsWeight = -9999;
    foreach (self::coreResourceList($region) as $item) {
      if (is_array($item)) {
        $bundle->addSetting($item);
      }
      elseif (strpos($item, '.css')) {
        CRM_Core_Resources::isFullyFormedUrl($item)
          ? $bundle->addStyleUrl($item, ['weight' => -100])
          : $bundle->addStyleFile('civicrm', $item, ['weight' => -100]);
      }
      elseif (CRM_Core_Resources::isFullyFormedUrl($item)) {
    ...
    // TODO?    $bundle->addCoreStyles($region);
    return self::useRegion($bundle, $region);
  }
  ...
  protected static function coreResourceList($region) {
    ...
    if ($editor == "CKEditor") {
      ...
      $items[] = [
        'config' => [
          'wysisygScriptLocation' => Civi::paths()
            ->getUrl("[civicrm.root]/js/wysiwyg/crm.ckeditor.js"),
    ...
    $map = [
      'bower_components' => rtrim(Civi::paths()
        ->getUrl('[civicrm.bower]/.', 'absolute'), '/'),
      'packages' => rtrim(Civi::paths()
        ->getUrl('[civicrm.packages]/.', 'absolute'), '/'),
    ];
