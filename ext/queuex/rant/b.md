...
Project Citges aims to change the way Civi handles background work - while supporting three styles of deployment:

| Style | Performance | Compatibility |
| -- | -- | --|
| Web Cron<br/> Cheap Hosting Architecture | Worst  | Best |
| CLI Cron<br/> Dedicated Hosting Architecture | Moderate | Moderate |
| Daemon<br/> Multitenant Hosting Architecture | Best | Worst |


* __Web Cron / Cheap Hosting Architecture__: The system
* __CLI Cron / Dedicated Hosting Architecture__: Run a `SoloGovernor` with `ForkWorker` via cron+CLI
* __Daemon / Multitenant Hosting Architecture__ Run an `ImmortalGovernor` for a multisite deployment.
