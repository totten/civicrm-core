 ...
 * extension.
 */
class CRM_Legacycustomsearches_ExtensionUtil {
  const SHORT_NAME = 'legacycustomsearches';
  const LONG_NAME = 'legacycustomsearches';
  const CLASS_PREFIX = 'CRM_Legacycustomsearches';
 ...
 */
function _legacycustomsearches_civix_civicrm_managed(&$entities) {
  $mgdFiles = _legacycustomsearches_civix_find_files(__DIR__, '*.mgd.php');
  sort($mgdFiles);
...
function _legacycustomsearches_civix_civicrm_caseTypes(&$caseTypes) {
  ...
  foreach (_legacycustomsearches_civix_glob(__DIR__ . '/xml/case/*.xml') as $file) {
 ...
 */
function _legacycustomsearches_civix_civicrm_themes(&$themes) {
  $files = _legacycustomsearches_civix_glob(__DIR__ . '/*.theme.php');
 ...
 * @return bool
 */
function _legacycustomsearches_civix_insert_navigation_menu(&$menu, $path, $item) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _legacycustomsearches_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
 ...
 */
function _legacycustomsearches_civix_navigationMenu(&$nodes) {
  if (!is_callable(['CRM_Core_BAO_Navigation', 'fixNavigationMenu'])) {
    _legacycustomsearches_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _legacycustomsearches_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _legacycustomsearches_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _legacycustomsearches_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _legacycustomsearches_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
