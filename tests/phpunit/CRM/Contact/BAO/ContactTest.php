...
class CRM_Contact_BAO_ContactTest extends CiviUnitTestCase {
  ...
  public function _testTimestamps($callbacks) {
    ...
    $this->assertRegex('/^\d\d\d\d-\d\d-\d\d /', $origTimestamps['created_date']);
    $this->assertRegex('/^\d\d\d\d-\d\d-\d\d /', $origTimestamps['modified_date']);
    ...
    foreach ($callbacks as $callbackName => $callback) {
      ...
      $this->assertRegex('/^\d\d\d\d-\d\d-\d\d /', $newTimestamps['created_date'], "Malformed created_date (after $callbackName)");
      $this->assertRegex('/^\d\d\d\d-\d\d-\d\d /', $newTimestamps['modified_date'], "Malformed modified_date (after $callbackName)");
