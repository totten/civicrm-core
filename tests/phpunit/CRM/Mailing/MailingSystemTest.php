 ...
 * @group headless
 */
class CRM_Mailing_MailingSystemTest extends CiviUnitTestCase {
   ...
   * @param $settings
   */
  public function testBasicHeaders($settings) {
   ...
   * @param $settings
   */
  public function testText($settings) {
    Civi::settings()->add($settings);
    $allMessages = $this->runMailingSuccess(array(
      'subject' => 'Accidents in cars cause children for {contact.display_name}!',
      'body_text' => 'BEWARE children need regular infusions of toys. Santa knows your {domain.address}. There is no {action.optOutUrl}.',
   ...
   * @param $settings
   */
  public function testHtmlWithOpenTracking($settings) {
    Civi::settings()->add($settings);
    $allMessages = $this->runMailingSuccess(array(
   ...
   * @param $settings
   */
  public function testHtmlWithOpenAndUrlTracking($settings) {
    Civi::settings()->add($settings);
    $allMessages = $this->runMailingSuccess(array(
      'subject' => 'Example Subject',
  ...
  public function urlTrackingExamples() {
    ...
    // Repeat each case with old and new mailers.
    $allCases = array();
    foreach ($cases as $case) {
      foreach ($this->basicSettings() as $settingsCase) {
        $case[] = $settingsCase[0];
        $allCases[] = $case;
      }
    }
   ...
   * @param $settings
   */
  public function testUrlTracking($inputHtml, $htmlUrlRegex, $textUrlRegex, $params, $settings) {
    $caseName = print_r(array('inputHtml' => $inputHtml, 'params' => $params, 'settings' => $settings), 1);
