...
class FieldSpecTest extends \CiviUnitTestCase {
  ...
  /**
   * Interim measure while naming convention is subject to change
   */
  public function testScopeAlias() {
    ...
    $this->assertEquals($interpret('tplParams'), $interpret('tplParams'));
    $this->assertEquals($interpret('tpl as fooBar'), $interpret('tplParams as fooBar'));
    $this->assertEquals($interpret('tokenParams'), $interpret('tokenContext'));
    $this->assertEquals($interpret(['tplParams' => 'zoo']), $interpret(['tplParams' => 'zoo']));
  }
